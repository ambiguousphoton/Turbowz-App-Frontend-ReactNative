import React, { useState, useEffect } from 'react';
import { View, Text, TouchableOpacity, Linking, Image, Dimensions } from 'react-native';


interface BannerAdProps {
  ad_id: number;
  title: string;
  redirect_url: string;
}

const BannerAdComponent = ({ ad_id, title, redirect_url }: BannerAdProps) => {
  const [imageError, setImageError] = useState(false);
  const [imageHeight, setImageHeight] = useState(200);
  const { width: screenWidth } = Dimensions.get('window');

  return (
    <TouchableOpacity 
      className="border-b border-gray-100"
      onPress={() => Linking.openURL(redirect_url)}
    >
      <View className="px-2 py-1 bg-primary-25">
        <Text className="text-xs text-gray-600">Promoted Content</Text>
      </View>
      {!imageError && (
        <Image 
          source={{ uri: `http://10.0.2.2:8088/ad?ad_id=${ad_id}` }}
          style={{ width: screenWidth, height: imageHeight }}
          resizeMode="contain"
          onError={(error) => {
            console.log('Ad image load error:', error.nativeEvent.error);
            console.log('Ad image URL:', `http://10.0.2.2:8088/ad?ad_id=${ad_id}`);
            setImageError(true);
          }}
          onLoad={(event) => {
            const { width, height } = event.nativeEvent.source;
            const aspectRatio = height / width;
            const calculatedHeight = screenWidth * aspectRatio;
            setImageHeight(calculatedHeight);
            console.log('Ad image loaded successfully for ad_id:', ad_id, 'calculated height:', calculatedHeight);
          }}
        />
      )}
      <View className="p-4 mb-8">
        <Text className="font-semibold text-gray-800">{title}</Text>
        <Text className="text-xs text-gray-500 mt-1">Advertisement</Text>
        <TouchableOpacity 
          className="bg-primary-25 border border-primary-200 px-4 py-2  rounded-lg mt-3 flex-row items-center"
          onPress={() => Linking.openURL(redirect_url)}
        >
          <Text className="text-primary-200 font-semibold text-base mr-2">Learn More !</Text>
        </TouchableOpacity>
      </View>
    </TouchableOpacity>
  );
};

export default BannerAdComponent;